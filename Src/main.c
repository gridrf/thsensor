    ...
    case 2:
        {
            ...
            temperature = ( int16_t )( SHT3XReadTemperature( ) * 100 );       // in °C * 100
            humidity = ( int16_t )( SHT3XReadHumidity());           // in m * 10
            batteryLevel = BoardGetBatteryLevel( );
					
            AppData[0] = 0x0; //send info
            AppData[1] = 0x2; //th device
            AppData[2] = 0x1; //ver
            AppData[3] = LEDReadState();
            AppData[4] = LEDReadBRT();
            AppData[5] = ( temperature >> 8 ) & 0xFF;
            AppData[6] = temperature & 0xFF;
            AppData[7] = ( humidity >> 8 ) & 0xFF;
            AppData[8] = humidity & 0xFF;
            AppData[9] = batteryLevel;
        }
        break;
    case 224:
...
static void OnUpdateLEDTimerEvent( void )
{
    TimerReset( &LedTimer);
	  BoardUpdateLed();
}
...
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    if( mcpsIndication->RxData == true )
    {
        ...
        case 2:
            ...
            {
							  uint8_t *enc = mcpsIndication->Buffer;
							  if(enc[0] != 0x1 || enc[1]!=0x2 || enc[2]!=0x1){ //write, th device, ver
								    break;
								}
								//Display_On_Off
							  LEDSetState(enc[3]);
...
int main( void )
{
    ...
    BoardInitPeriph( );
	  ...
	  TimerInit( &LedTimer, OnUpdateLEDTimerEvent);
	  TimerSetValue( &LedTimer, 10000);
	  TimerStart( &LedTimer );
